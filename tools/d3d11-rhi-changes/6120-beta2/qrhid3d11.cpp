...
bool QD3D11SwapChain::createOrResize()
{
    ...
    // [*] Decide if flip swap chain model is used on `createOrResize` and use
    //     sequential swap effect.
    //     This is the 2nd of a 3-part workaround of `QQuickWindow` with D3D11
    ...
    if (!rhiD->useLegacySwapchainModel) {
        if (qEnvironmentVariableIsSet("QT_D3D_MAX_FRAME_LATENCY"))
            rhiD->maxFrameLatency = UINT(qMax(0, qEnvironmentVariableIntValue("QT_D3D_MAX_FRAME_LATENCY")));
    } else {
        rhiD->maxFrameLatency = 0;
    }
    ...
    qCDebug(QRHI_LOG_INFO, "FLIP_* swapchain supported = true, ALLOW_TEARING supported = %s, use legacy (non-FLIP) model = %s, max frame latency = %u",
            rhiD->supportsAllowTearing ? "true" : "false",
            rhiD->useLegacySwapchainModel ? "true" : "false",
            rhiD->maxFrameLatency);
    if (rhiD->maxFrameLatency == 0)
    ...
    if (!swapChain) {
        ...
        desc.BufferCount = rhiD->useLegacySwapchainModel ? 1 : BUFFER_COUNT;
        desc.Flags = swapChainFlags;
        desc.Scaling = rhiD->useLegacySwapchainModel ? DXGI_SCALING_STRETCH : DXGI_SCALING_NONE;
        desc.SwapEffect = rhiD->useLegacySwapchainModel ? DXGI_SWAP_EFFECT_SEQUENTIAL : DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL;
        desc.Stereo = stereo;
        ...
        hr = swapChain->ResizeBuffers(rhiD->useLegacySwapchainModel ? 1U : UINT(BUFFER_COUNT), UINT(pixelSize.width()), UINT(pixelSize.height()),
