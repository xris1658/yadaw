...
void dumpItemNames(QQuickItem* contentItem, std::uint32_t indent = 0)
{
    auto metaObject = contentItem->metaObject();
    FOR_RANGE0(i, indent)
    {
        std::fprintf(stderr, "  ");
    }
    auto name = contentItem->objectName().toLocal8Bit();
    std::fprintf(stderr, "%p", static_cast<void*>(contentItem));
    while(metaObject)
    {
        std::fprintf(stderr, ": %s", metaObject->className());
    ...
    {
        dumpItemNames(childItem, indent + 1);
    }
}

void dumpObjectNames(QObject* object, std::uint32_t indent = 0)
{
    ...
    auto name = object->objectName().toLocal8Bit();
    std::fprintf(stderr, "Class names of item %p", static_cast<void*>(object));
    while(metaObject)
    {
        std::fprintf(stderr, ": %s", metaObject->className());
        metaObject = metaObject->superClass();
...
bool NativePopupEventFilter::eventFilter(QObject* watched, QEvent* event)
{
    if(watched == parentWindow_.window)
    {
        ...
        // Since key events are not delivered to an inactive `QQuickWindow`, we
